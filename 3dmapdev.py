...
def plot_layers_3d(layers, season, flight, intersection_indices):
    ...
    if plot_surface:
        ...
        ax.plot(lons_surf[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset],
                lats_surf[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset],
                [-x for x in (twtts_surf[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset])], label='top. seg. 1', c='blue',
                linewidth=1)

        ax.plot(lons_surf[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset],
                lats_surf[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset],
                [-x for x in (twtts_surf[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset])], label='top seg. 2', c='black',
                linewidth=1)

        # draw a line from one end of the segment to the other in blue
        # Define the slice range
        start_index = intersection_indices[i][0] - offset
        end_index = intersection_indices[i][0] + offset
    ...
    start_index_seg1 = intersection_indices[i][1] - offset
    end_index_seg2 = intersection_indices[i][1] + offset

    start_index_seg2 = intersection_indices[i][0] - offset
    end_index_seg1 = intersection_indices[i][0] + offset

    # plot the bottom layer (layer 1)
    # ax.plot(lons_bottom[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset],
    #         lats_bottom[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset],
    #         [-x for x in (twtts_bottom[intersection_indices[i][0] - offset:intersection_indices[i][0] + offset])],
    #         label='bott. seg. 1', c='orange',
    #         linewidth=1)
    #
    # ax.plot(lons_bottom[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset],
    #         lats_bottom[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset],
    #         [-x for x in (twtts_bottom[intersection_indices[i][1] - offset:intersection_indices[i][1] + offset])],
    ...
    if plot_all:
        ...
        ax.plot(lons_bottom[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset],
                lats_bottom[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset],
                [-x for x in (twtts_bottom[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset])], label='whole bottom', c='purple',
                linewidth=0.25)

        # plot the rest of the surface layer
        ax.plot(lons_surf[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset],
                lats_surf[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset],
                [-x for x in (twtts_surf[intersection_indices[0][0] - offset:intersection_indices[0][1] + offset])], label='whole surface', c='grey',
                linewidth=0.25)
    ...
    ax.scatter(intersection_points[i][1], intersection_points[i][0], -1 * twtts_bottom[intersection_indices[i][0]],
               c='red', marker='o')
    ax.scatter(intersection_points[i][1], intersection_points[i][0], -1 * twtts_bottom[intersection_indices[i][1]],
               c='black', marker='o')

    ax.scatter(layers[1].lon[intersection_indices[i][0]],
               layers[1].lat[intersection_indices[i][0]],
               -1 * layers[1].twtt_corrected[intersection_indices[i][0]], c='green', marker='o')
    ax.scatter(layers[1].lon[intersection_indices[i][1]],
                 layers[1].lat[intersection_indices[i][1]],
                 -1 * layers[1].twtt_corrected[intersection_indices[i][1]], c='blue', marker='o')

    print(f"by way of twtt_bottom: \t{twtts_bottom[intersection_indices[i][0]]}")  # red one
    print(f"by way of twtt_bottom: \t{twtts_bottom[intersection_indices[i][1]]}")  # black one
    print(f"by way of twtt_corrected: \t{layers[1].twtt_corrected[intersection_indices[i][0]]}")  # green one
    print(f"by way of twtt_corrected: \t{layers[1].twtt_corrected[intersection_indices[i][1]]}")  # blue one
